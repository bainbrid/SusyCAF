  ...
  if jobnumber == str(row['rowid']):
    ...
    if len(datasets) > 1 :
      for dset in datasets :
        ...
        path = path.replace('/pnfs/cms/WAX/11','')
        print path
        paths.append(path)
        ...
        shortName = string.replace(shortName,'=','_')
        shortName = string.replace(shortName,' ','_')
        shortName = shortName+"_"+user+"Job"+jobnumber
...
#  prefix = '\n' + '\t' +  "\"dcap://cmsdca.fnal.gov:24137/pnfs/fnal.gov/usr/cms/WAX/11"# + path + '/'
  ...
  if method in ["y","yes"] : ### srmls
    ...
    while True :
      ...
      temp = commands.getstatusoutput("srmls -count %i -offset %i srm://cmssrm.fnal.gov:8443/11/%s" % (count,offset,path))
      print "srmls -count %i -offset %i srm://cmssrm.fnal.gov:8443/11/%s" % (count,offset,path)
      #Succesful?
  ...
  ##end srmls
    ...
    temp = commands.getstatusoutput("lcg-ls srm://cmssrm.fnal.gov:8443/11%s" + path + "/")
    #Succesful?
    if temp[0] != 0 :
      print "\tError occured:"
      print temp
