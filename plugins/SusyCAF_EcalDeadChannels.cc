...
SusyCAF_EcalDeadChannels::SusyCAF_EcalDeadChannels(const edm::ParameterSet& conf) {
  ...
  produces <std::vector<unsigned> > ( "ecalDeadTowerTrigPrimFound" );
}
...
produce(edm::Event& e, const edm::EventSetup& es) {
  ...
  for (std::vector<towerInfo>::const_iterator it = badTowers.begin(); it!=badTowers.end(); it++) {
    ...
    if(tpDigis.isValid()) {
      ...
      if (found) tpEt = fabs(ecalScale_.getTPGInGeV( tp->compressedEt(), tp->id() )) ;
    }
...
updateBadTowers(const edm::EventSetup& es) {
  edm::ESHandle<EcalChannelStatus> channelStatus;  es.get<EcalChannelStatusRcd>().get(channelStatus);
  edm::ESHandle<CaloGeometry> caloGeometry;        es.get<CaloGeometryRecord>().get(caloGeometry);
  ...
  for(std::map<uint32_t,unsigned>::const_iterator it = nBadXtal.begin(); it!=nBadXtal.end(); it++) {
    ...
    EcalTrigTowerDetId id(key);
    const GlobalPoint& point = caloGeometry->getPosition(id);
    badTowers.push_back( towerInfo( key, nBadXtal[key], maxStatus[key], point.eta(), point.phi())  );
  }
}
