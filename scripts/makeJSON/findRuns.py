...
#DK
CFGFILE = CONFIG.get('Common','CFGFILE')
...
#DK
LISTOFRUN_NONEPOG=[]
for line in run_data_NONEPOG.split("\n"):
    run=line.split(',')[0]
    if run.isdigit():
...
if CFGFILE != "NONE":
    CFG_lumiSummary = open(CFGFILE, 'w')
    CFG_lumiSummary.write(selectlumi)
    CFG_lumiSummary.close()
    print
    print "        cfg snippet: ",CFGFILE

# buildup cms snippet for bad POG
selectlumi="process.source.lumisToProcess = cms.untracked.VLuminosityBlockRange(\n"
ranges = []
runs_to_print = selected_dcs_NONEPOG.keys()
runs_to_print.sort()
for run in runs_to_print:
   blocks = selected_dcs_NONEPOG[run]
   blocks.sort()
   prevblock = [-2,-2]
   for lsrange in blocks:
       if lsrange[0] == prevblock[1]+1:
...
if CFGFILE != "NONE":
    if len(LISTOFRUN_NONEPOG) > 0:
       CFGFILE="NONEPOG_"+CFGFILE
       CFG_lumiSummary = open(CFGFILE, 'w')
       CFG_lumiSummary.write(selectlumi)
       CFG_lumiSummary.close()
       print "bad POG cfg snippet: ",CFGFILE
    print "-------------------------------------------"
